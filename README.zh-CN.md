...
每条消息的流程：若不是发给机器人的群消息，则写入记录并生成嵌入 →
若是发给它的，用一次严格 JSON 调用判断意图，然后回复。**没有被 @ 时，它不出声。**
...
成员画像的区分度、按人锚定的「我错过了什么」、承诺在重启后仍然生效、
备份与恢复、向量检索往返，以及参考资料目录的范围隔离。
